...
def prepare_files_for_consensus(config):
    ...
    print >> ff, '#program     isomerID     nposes'
      
    sh = 0 # shift of model idx
    for program in config.programs:
        for jdx in range(config.nisomers):

            if config.nisomers == 1:
                prgdir = '../%s'%program
            else:
                prgdir = '../%s.%s'%(program,jdx+1)
            ...
            with open(prgdir+'/rec-lig.out.pdb') as pdbfi:
                for line in pdbfi:
                    if line.startswith('MODEL'):
                        ...
                        pdbfo.write(line)
                sh += idx
                print >> ff, '%10s       %s        %s'%(program,jdx+1,idx)
    pdbfo.close()
    nposes = sh
...
def run_tleap(config, nposes):
    ...
    if config.nisomers == 1:
        ...
        prgdir = '../../%s.%s'%(config.programs[0],1)
...
cluster ":LIG & !@/H" nofit mass epsilon 3.0 summary summary.dat info info.dat
"""% locals()
        file.write(script)
